...
import { fetchHsk20Lessons } from "@/lib/api";
import { MainTabParamList, RootStackParamList } from "@/navigation/AppNavigator";
import type { LessonSummary } from "@/types";

type Props = CompositeScreenProps<
  BottomTabScreenProps<MainTabParamList, "HSK20">,
...
export function HSK20Screen({ navigation }: Props) {
  ...
  async function loadLessons(isRefresh = false) {
    try {
      ...
      const allResponse = await fetchHsk20Lessons();
      ...
      const filteredResponse = await fetchHsk20Lessons({
        level: effectiveLevel,
        q: query,
      });

      setAllLessons(allResponse.lessons);
      setLessons(filteredResponse.lessons);
      ...
      <View style={[styles.header, { paddingTop: insets.top + 12 }]}>
        <View style={styles.headerBar}>
          <View style={styles.headerText}>
            <Text style={styles.title}>HSK 2.0</Text>
          </View>
          ...
          renderItem={({ item }) => (
            ...
            >
              <View style={styles.cardTop}>
                ...
                <Text style={styles.order}>Bài {item.orderNo}</Text>
              </View>
              <Text style={styles.cardTitle}>{item.title}</Text>
              <Text style={styles.cardDesc}>{item.description || "Bài học HSK 2.0."}</Text>
